...
from tetris_pg_feature import TetrisApp
import copy
...
class DuelingDoubleDQNagent():
    def __init__(self):
        ...
        self.beta = 0.4 # 0.4
        self.beta_max = 1.0
        self.beta_decay = 3000000 #5000000
    ...
    def build_model(self):
        ...
        layer = Dense(512, activation='relu', kernel_initializer='he_uniform')(state)
        ...
        vlayer = Dense(128, activation='relu', kernel_initializer='he_uniform')(layer)
        alayer = Dense(128, activation='relu', kernel_initializer='he_uniform')(layer)
...
class DQN():
    ...
    def run(self):
        ...
        for e in range(EPISODES):
            ...
            state = env.whole_features
            state = np.reshape([state], (feature_len, 1))
        
            while not done:

                self.global_step += 1
                
                action = self.agent.get_action(np.reshape(state, [1, feature_len, 1]))
                ...
                if env.gameover:
                    ...
                    stats = [env.score, self.global_step]
                ...
                next_state = env.whole_features
                next_state = np.reshape([next_state], (feature_len, 1))
            ...
            print("episode:", e, "score:", score, "total_clline:", env.total_clline, "global_step:", self.global_step, "epsilon:", self.agent.epsilon, "beta:", self.agent.beta)
        

            # 10000000번의 행동을 취한 후에 학습 종료
            if e%1000 == 0 and e>10000:
                self.agent.model.save_weights("./DQN_tetris_model.h5")

            if score > 200:
                self.agent.model.save_weights(".DQN_tetris_model_best")
