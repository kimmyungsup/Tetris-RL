...
class TetrisApp(object):
    ...
    # stone generate : random or fixed
        ...
        self.next_stone_num = self.shapes.pop(rand(len(self.shapes)))
        self.next_stone = tetris_shapes[self.next_stone_num]
        self.new_stone_flag = False

        # value for transport next stone
    ...
    def new_stone(self):
        ...
        self.current_stone = self.next_stone_num
    ...
    def init_game(self):
        ...
        # feature definition
        holes = self.num_hole(self.board) / 20
        column_height = [0 for _ in range(cols)]
        for m in range(cols):
            for n in range(rows):
                if self.board[n][m] > 0:
                    column_height[m] = (rows - n) / rows
                    break
        max_height = max(column_height) / rows
        column_diff = [0 for _ in range(cols - 1)]
        for m in range(cols - 1):
            column_diff[m] = abs(column_height[m] - column_height[m + 1])
        features = [holes] + column_height + [max_height] + column_diff  # basic feature  ( 1 + 7 + 1 + 6 = 15 )

        block_shape = [0 for _ in range(len(tetris_shapes))]
        block_shape[self.current_stone] = 1


        features = features + block_shape + [self.stone_x, self.stone_y] + [
            self.next_stone_num / 7]  # information of stone
        # length = 25
        self.whole_features = features
            ...
            if not self.gameover:
                ...
                if check_collision(self.board,
                    ...
                    if self.num_hole(self.board) < cur_hole:
                        self.plus_score += (cur_hole-self.num_hole(self.board))
    ...
    def step(self, action):
        ...
        features = features + block_shape + [self.stone_x, self.stone_y] + [self.next_stone_num / 7]  # information of stone
        # length = 25
        self.whole_features = features
        ...
        return reward, board_screen, features###, aa
                ...
                if event.type == pygame.USEREVENT + 1:
                    ...
                    for key in key_actions:
                        ...
                        if event.key == eval("pygame.K_"
                            ...
                            # feature definition
                            print("############# features ################")
                            holes = self.num_hole(self.board)
                            print("holes : ", holes)
                            column_height = [0 for _ in range(cols)]
                            for m in range(cols):
                                for n in range(rows):
                                    if self.board[n][m] > 0:
                                        column_height[m] = rows - n
                                        break
                            print("column_height : ", column_height)
                            max_height = max(column_height)
                            print("max_height : ", max_height)
                            column_diff = [0 for _ in range(cols - 1)]
                            for m in range(cols - 1):
                                column_diff[m] = abs(column_height[m] - column_height[m + 1])
                            print("column_difference : ", column_diff)

                            block_shape = [0 for _ in range(len(tetris_shapes))]
                            block_shape[self.current_stone] = 1
                            print("block_shape : ", block_shape)

                            print("next_stone : ", self.next_stone_num)

                            features = [holes] + column_height + [max_height] + column_diff
                            features = features + block_shape + [self.stone_x, self.stone_y] + [self.next_stone_num]
                            print("whole features : ", features)
                            print("feature lenth : ", len(features)+7)
